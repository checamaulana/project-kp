...
## 5. Fase 2: MVP — Modul Permintaan Pelayanan (5 hari)

### 5.1 Hari 14: Setup Pelayanan

#### Task 2.1.1: Migration + Model Pelayanan
- **Estimasi:** 2 jam
- **Steps:**
  1. `php artisan make:migration create_pelayanans_table` + tulis.
  2. `php artisan make:migration create_pelayanan_progress_table` + tulis.
  3. `php artisan make:model Pelayanan -mf` + relasi.
  4. `php artisan make:model PelayananProgress -mf`.
  5. `php artisan make:enum StatusPelayananEnum`.
- **Acceptance:** Tabel ada, model + relasi OK.

#### Task 2.1.2: PelayananService
- **Estimasi:** 3 jam
- **Steps:**
  1. `php artisan make:request PelayananStoreRequest` + rules (judul max 100 char, dll).
  2. `php artisan make:request PelayananProgressRequest`.
  3. `php artisan make:service PelayananService` + create, update progress, close.
  4. `php artisan make:policy PelayananPolicy`.
- **Acceptance:** CRUD + progress flow berfungsi.

#### Task 2.1.3: PelayananController + ProgressController
- **Estimasi:** 2 jam
- **Steps:**
  1. `php artisan make:controller PelayananController --resource`.
  2. `php artisan make:controller PelayananProgressController` (method `store` only).
  3. Register routes.
- **Acceptance:** Routes berfungsi.

#### Task 2.1.4: Notifikasi Pelayanan
- **Estimasi:** 1 jam
- **Steps:**
  1. `php artisan make:notification PelayananBaruNotification`.
  2. `php artisan make:notification PelayananProgressNotification`.
  3. Wire ke Service.
- **Acceptance:** Notif terkirim saat tiket baru & progress update.

### 5.2 Hari 15-16: Frontend Pelayanan

#### Task 2.2.1: Setup Tiptap rich text editor
- **Estimasi:** 2 jam
- **Steps:**
  1. Install Tiptap + extensions (StarterKit, Link, Image).
  2. Buat `components/common/RichTextEditor.tsx` (reusable).
  3. Setup sanitization dengan DOMPurify saat render.
- **Acceptance:** Editor berfungsi, output sanitized HTML.

#### Task 2.2.2: Form Pengajuan
- **Estimasi:** 3 jam
- **Steps:**
  1. Buat `components/pelayanan/PelayananForm.tsx`.
  2. Field: judul, jenis pelayanan (dropdown), aplikasi (dropdown), detail (rich text), lampiran (multi-file).
  3. Buat `pages/Pelayanan/Create.tsx`.
- **Acceptance:** User bisa ajukan tiket.

#### Task 2.2.3: Daftar + Rincian Pelayanan
- **Estimasi:** 4 jam
- **Steps:**
  1. Buat `pages/Pelayanan/Index.tsx` (tabel + filter).
  2. Buat `pages/Pelayanan/Show.tsx` (info + deskripsi + riwayat progress).
  3. Buat `components/pelayanan/PelayananProgress.tsx` (timeline progress).
  4. Buat `components/pelayanan/ProgressForm.tsx` (handler).
- **Acceptance:** User bisa lihat tiket, handler bisa update progress.

#### Task 2.2.4: Test Pelayanan
- **Estimasi:** 2 jam
- **Steps:**
  1. Feature test: create, progress, close, authorization.
- **Acceptance:** Coverage > 70%.
